...
Overture lives at `plugins/overture/` in the
[orchestrator monorepo](../../README.md); the marketplace manifest that makes
the repo installable sits at the repo root. Paths below are relative to this
plugin directory.
...
From this directory (`plugins/overture/`):

```bash
claude plugin validate .                      # the resolved plugin
